...
public class MIN {
	...
	public void newPage(char[] arr, int i, int x){
		//if there is no row which has not been filled yet
		if(active.size() > (rows - 1)){ 
			
			char toRemove = minRemove();
			String temp = (String) getKeyFromValue(map,map.get(Character.toString((char) active.get(active.indexOf(toRemove)))));
			map.put(Character.toString(arr[i]) , map.get(temp));
			map.remove(temp);
			active.remove(active.indexOf(toRemove));
			active.add(arr[i]);
			String newtemp = map.get(Character.toString(arr[i]));
	 ...
	 public char minRemove(){
		 char toRemove = 0;
			List<Character> minArray = new ArrayList<Character>();
			for(char elem : patList){
				if(minArray.size() < rows){
					if(active.contains(elem) & !(minArray.contains(elem))){
						minArray.add(elem);
					}
				}
			}
			if(minArray.size() == 0){
				toRemove = active.get(0);
			}
			if(minArray.size() > 0){
				toRemove = minArray.get((minArray.size() - 1));
			}
			if (minArray.size() < active.size()){
				for(char add : active){
					if(!(minArray.contains(add))){
						minArray.add(add);
					}
				}
				toRemove = minArray.get((minArray.size() - 1));
